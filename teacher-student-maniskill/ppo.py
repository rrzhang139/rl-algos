...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_mean = nn.Sequential(
            ...
            layer_init(nn.Linear(256, 256)),
            nn.Tanh(),
            layer_init(nn.Linear(256, np.prod(
                envs.single_action_space.shape)), std=0.01*np.sqrt(2)),
        )
        self.actor_logstd = nn.Parameter(torch.ones(
            1, np.prod(envs.single_action_space.shape)) * -0.5)
...
if __name__ == "__main__":
    ...
    # env setup
    env_kwargs = dict(obs_mode="state", render_mode="rgb_array",
    ...
    if args.capture_video:
        ...
        if args.save_train_video_freq is not None:
            def save_video_trigger(x): return (
    ...
    print(f"####")
    action_space_low, action_space_high = torch.from_numpy(envs.single_action_space.low).to(
        device), torch.from_numpy(envs.single_action_space.high).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            next_done = torch.logical_or(
                terminations, truncations).to(torch.float32)
            ...
            if "final_info" in infos:
                ...
                with torch.no_grad():
                    final_values[step, torch.arange(args.num_envs, device=device)[
                        done_mask]] = agent.get_value(infos["final_observation"][done_mask]).view(-1)
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                real_next_values = next_not_done * nextvalues + \
                    final_values[t]  # t instead of t+1
                ...
                if args.finite_horizon_gae:
                    ...
                    reward_term_sum = args.gae_lambda * args.gamma * \
                        reward_term_sum + lam_coef_sum * rewards[t]
                    value_term_sum = args.gae_lambda * args.gamma * \
                        value_term_sum + args.gamma * real_next_values

                    advantages[t] = (reward_term_sum +
                                     value_term_sum) / lam_coef_sum - values[t]
                else:
                    delta = rewards[t] + args.gamma * \
                        real_next_values - values[t]
                    ...
                    advantages[t] = lastgaelam = delta + args.gamma * \
                        args.gae_lambda * next_not_done * lastgaelam
        ...
        logger.add_scalar("time/rollout_fps", args.num_envs *
                          args.num_steps / rollout_time, global_step)
    if not args.evaluate:
        if args.save_model:
